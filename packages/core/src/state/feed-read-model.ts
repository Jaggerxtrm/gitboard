...
export type FeedSource = "specialists" | "beads" | "github" | "materializer";
export type FeedSeverity = "debug" | "info" | "warn" | "error";
export type FeedRedactionStatus = "clean" | "redacted";
...
export interface FeedRow {
  ...
  status: string;
  redaction_status: FeedRedactionStatus;
  drilldown: {
    job_id?: string;
    issue_id?: string;
...
export function parseFeedCursor(value: string | undefined | null): FeedCursor | null {
...
export function encodeFeedCursor(row: FeedRow): string {
  return Buffer.from(JSON.stringify({ t_unix_ms: row.t_unix_ms, seq: row.seq, id: row.id }), "utf8").toString("base64url");
}

export function normalizeFeedLimit(value: number | string | undefined | null): number {
...
function normalizeSeverity(value: unknown): FeedSeverity {
  return value === "debug" || value === "info" || value === "warn" || value === "error" ? value : "info";
}

function normalizeRedaction(value: unknown): FeedRedactionStatus {
