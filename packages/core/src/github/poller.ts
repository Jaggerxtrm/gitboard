...
import { makeSourceHealth, type SourceHealth, type SourceHealthStatus } from "../state/source-health.ts";
import {
  type GithubActivityPublisher,
  type GithubAdapterChannelName,
  type GithubAdapterEventName,
  type GithubAdapterLogEntry,
  type GithubAdapterLogger,
  makeGithubAdapterLogEntry,
  NOOP_GITHUB_ACTIVITY_PUBLISHER,
  NOOP_GITHUB_ADAPTER_LOGGER,
} from "./ports.ts";
...
export class GithubPoller {
  ...
  private registry: GithubActivityPublisher;
  private logger: GithubAdapterLogger;
  ...
  constructor(db: Database, token: string, options: GithubPollerOptions = {}) {
    ...
    this.registry = options.registry ?? NOOP_GITHUB_ACTIVITY_PUBLISHER;
  ...
  private emitLog(component: "poller" | "github" | "api", event: string, level: "debug" | "info" | "warn" | "error", msg?: string, data?: Record<string, unknown>): void {
    const entry: GithubAdapterLogEntry = makeGithubAdapterLogEntry(component, event, level, msg, data);
    this.logger.emit(entry);
  }

  private parseRateLimit(response: Response): RateLimitSnapshot | null {
    ...
    return { remaining, limit, reset_at: new Date(resetSeconds * 1000).toISOString() };
  }
  ...
  private maybePauseForRateLimit(response: Response): boolean {
    ...
    if ((response.status === 403 || response.status === 429) && Number.isFinite(retryAfter)) {
      ...
      this.publishGithubSourceHealth("degraded", { status: response.status, retryAfter, pausedUntil: this.pausedUntil, ...(rate ?? {}) });
      this.emitLog("poller", "rate_limit.changed", "warn", undefined, { status: response.status, retryAfter, pausedUntil: this.pausedUntil });
      return true;
    }
    if ((response.status === 403 || response.status === 429) && Number.isFinite(resetSeconds)) {
      ...
      const reset_at = new Date(resetSeconds * 1000).toISOString();
      this.publishGithubSourceHealth("degraded", { status: response.status, resetSeconds, pausedUntil: this.pausedUntil, reset_at, ...(rate ?? {}) });
      this.emitLog("poller", "rate_limit.changed", "warn", undefined, { status: response.status, resetSeconds, pausedUntil: this.pausedUntil });
      return true;
    }
    ...
    if (rate.remaining < 500) {
      ...
      this.publishGithubSourceHealth("degraded", { ...rate, pausedUntil: this.pausedUntil });
      this.emitLog("poller", "rate_limit.changed", "warn", undefined, { remaining: rate.remaining, limit: rate.limit, pausedUntil: this.pausedUntil });
      return true;
    }
    if (this.pausedUntil > 0 && this.pausedUntil <= Date.now() && rate.remaining > rate.limit * 0.8) {
      this.pausedUntil = 0;
      this.publishGithubSourceHealth("fresh", rate);
    }
    return false;
  }
  ...
  private async apiGetWithMeta<T>(path: string, repo = "global", endpoint = path, persistedEtag?: string | null): Promise<ApiGetResult<T>> {
    ...
    if (response.status === 304) {
      ...
      this.emitLog("poller", "etag.hit_304", "debug", undefined, { repo, endpoint });
      return { data: null, status: "not_modified", etag: responseEtag };
    }
    if (!response.ok) {
      this.maybePauseForRateLimit(response);
      this.emitLog("poller", "etag.miss", "debug", undefined, { repo, endpoint, status: response.status });
  ...
  private getRepoActiveWindow(repo: GithubRepo, state: RepoPollState): number {
  ...
  private publishGithubEvent(event: GithubAdapterEventName, data: GithubPr | GithubIssue | GithubRelease | GithubEvent | SourceHealth | Record<string, unknown>, version?: string): void {
    this.registry.publish("github:activity", event, data, version ?? this.protocolVersion);
  }

  private publishGithubSourceHealth(status: SourceHealthStatus, metadata: Record<string, unknown>): void {
    const rateLimit = rateLimitFromMetadata(metadata);
    const base = makeSourceHealth("github", status, { metadata });
  ...
  private isRepoDue(repo: GithubRepo, state: RepoPollState, now = Date.now()): boolean {
  ...
  async ingestEvents(rawEvents: RawGithubEvent[]): Promise<void> {
    ...
    if (newEvents > 0 || newCommits > 0) {
      this.emitLog("poller", "ingest.summary", "info", undefined, { newEvents, newCommits });
    }
  }
  ...
  async pollUser(username: string): Promise<void> {
    ...
    if (!response.ok) {
      ...
      if (remaining && parseInt(remaining) < 50) {
        this.emitLog("poller", "rate_limit.low", "warn", undefined, { remaining });
      }
    ...
    if (remaining && limit) {
      ...
      if (ratio < 0.1) {
        this.emitLog("poller", "rate_limit.backoff", "warn", undefined, { ratio });
  ...
  async backfill(username: string): Promise<void> {
    this.emitLog("poller", "backfill.start", "info", undefined, { username, pages: this.backfillPages });
    const startedAt = Date.now();
    this.emitLog("poller", "cycle.start", "info", undefined, { username, phase: "backfill" });
    ...
    for (let page = 1; page <= this.backfillPages; page++) {
      ...
      try {
        ...
        this.emitLog("poller", "backfill.network_error", "warn", undefined, { username, page });
        break;
      }
      if (!response.ok) {
        this.emitLog("poller", "backfill.failed", "warn", undefined, { username, page, status: response.status });
        break;
      }
      const events = (await response.json()) as RawGithubEvent[];
    ...
    this.emitLog("poller", "cycle.complete", "info", undefined, { username, phase: "backfill", repos: repos.length, durationMs: Date.now() - startedAt });
  }

  start(username: string): void {
    if (this.timer) return;
    this.emitLog("poller", "poll_loop.start", "info", undefined, { username, intervalMs: this.intervalMs });
    this.timer = setInterval(async () => {
      const startedAt = Date.now();
      this.emitLog("poller", "cycle.start", "info", undefined, { username, phase: "poll" });
      try {
        await this.pollUser(username);
        await this.pollRepos();
        this.emitLog("poller", "cycle.complete", "info", undefined, { username, phase: "poll", durationMs: Date.now() - startedAt });
      } catch (err) {
        this.emitLog("poller", "error", "error", "poll loop failed", { error: (err as Error).message });
      }
    }, this.intervalMs);
  }
  ...
  stop(): void {
    if (this.timer) {
      ...
      this.timer = null;
      this.emitLog("poller", "poll_loop.stop", "info");
    }
  }
}

function rateLimitFromMetadata(metadata: Record<string, unknown>): { limit: number; remaining: number; reset_at: string } | undefined {
  const remaining = Number(metadata.remaining);
  const limit = Number(metadata.limit);
  const resetAt = typeof metadata.reset_at === "string" ? metadata.reset_at : undefined;
  if (!Number.isFinite(remaining) || !Number.isFinite(limit) || !resetAt) return undefined;
  return { remaining, limit, reset_at: resetAt };
}

// Optional entry point: bun run packages/core/src/github/poller.ts
// Apps that need a one-shot poller CLI must wire their own database factory.
if (import.meta.main) {
  throw new Error("packages/core/src/github/poller.ts no longer ships a CLI entry point. Wire the poller from your app or daemon entry instead.");
}
