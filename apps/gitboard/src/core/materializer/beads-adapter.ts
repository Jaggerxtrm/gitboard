...
export class BeadsAdapter implements MaterializerAdapter<MaterializedIssue, MaterializedDependency> {
  ...
  private async readSnapshotIssues(): Promise<{ rows: MaterializedIssue[]; dependencies: MaterializedDependency[] }> {
    ...
    return { rows, dependencies: issues.flatMap((issue) => materializeEdges(this.options.projectId, issue)) };
  }
...
function materializeEdges(projectId: string, issue: BeadIssue): MaterializedDependency[] {
...
function deriveRuntimeKind(issue: BeadIssue, labels: readonly string[]): string {
...
function buildMetadataJson(issue: BeadIssue, labels: readonly string[]): string | null {
