...
Agent Forge is a CLI/TUI orchestrator for AI agents managing Mercury stack services. The current implementation covers the **GitHub Activity Dashboard** (v0.7.0) — a web dashboard showing GitHub events, commits, and contribution data pulled via the GitHub API.
...
The application runs as a **single Bun process** that combines the API server, GitHub poller, and (in production) static file serving. In development, Vite runs separately and proxies API calls.
...
- **`store.ts`** — `createDatabase(path)` opens/creates `state.db` with WAL mode and all 6 tables (`sessions`, `messages`, `specialist_events`, `github_events`, `github_commits`, `github_repos`).
- **`github-store.ts`** — typed CRUD functions over the 3 GitHub tables: insert/get events, commits, repos; `getContributions(db, weeks)`, `getSummary(db, period)`.
...
- **`server.ts`** — `createApp(db)` returns `{ app, registry, wsHandler }`; `startServer(db, options)` calls `Bun.serve`. CORS enabled, health check at `GET /health`.
- **`routes/github.ts`** — 9 REST endpoints under `/api/github/` (events, commits, repos, contributions, summary — see HANDOFF.md for the full table).
- **`ws/channels.ts`** — `ChannelRegistry` pub/sub. Channel `github:activity` pushes `new_event`/`new_commits` messages.
- **`ws/handler.ts`** — `WsHandler` connection lifecycle. Client messages: `{ action: 'subscribe', channel: '...' }`.

### Dashboard UI (`src/dashboard/`)

React 19 SPA, Vite 7 build, Tailwind CSS v4 (uses `@import "tailwindcss"` syntax — requires `@tailwindcss/vite` plugin).

Key conventions:
- **`lib/`** not `api/` — renamed to avoid Vite proxy collision with the `/api` proxy rule.
- Icons: `@primer/octicons-react` only — no emoji, no custom SVGs.
- Lists: `@tanstack/react-virtual` for virtualized event lists.
...
- Dashboard tests (`tests/dashboard/`): `environment: "happy-dom"` (configured via `environmentMatchGlobs`).
- Setup file: `tests/dashboard/setup.ts`.
...
- Agent execution (`AgentSession` interface, `PiAgentSession`) is v0.3.0+ — do not scaffold it yet.
